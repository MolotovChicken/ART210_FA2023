...
//ALL PARTS OF swarm are referenced heavily, then modified, from in-class example
let agents = [];
...
function setup() {
  ...
  //resized because I exported it at 2x scale, must be halved to fit
  whaleImg.resize(1280, 800);
  
  //part of particle swarm code
  for (let i = 0; i < 40; i++){
...
function draw() {
  ...
  //another part of particle swarm code
  for (let agent of agents){
    agent.update();
    agent.show();
  }
  //Overall adjustment to make background lighter so hagfish (squigglies) can be seen
  //a white rectangle over entire screen, low opacity
  fill(255, 15);
  noStroke();
  rect(0, 0, 1280, 800);
  ...
  //a and b will stay between ___ and ___
  if (a > 360 || a < 200){
    aStep *= -1;
  } 
  if (b > 360 || b < 200){
...
class Agent {
  constructor(){
    ...
    this.xoff = random(60);
    this.yoff = random(60);
    this.color = [100, 200]; //slightly tansparent grey
  }
  update(){
    this.x = map(noise(this.xoff), 0, 1, 0, width);
    this.y = map(noise(this.yoff), 0, 1, 0, height);
    this.xoff += 0.0002; //velocity, very small to reflect underwater movement
    this.yoff += 0.0002;
  }
  show(){
    fill(this.color);
